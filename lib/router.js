...
methods.forEach(function (method) {
  Router.prototype[method] = function (options) {
    ...
    if (opts instanceof RegExp || typeof opts === 'string') {
      ...
      throw new TypeError('url (string) required');
    }
 ...
 */
Router.prototype.add = function (url, router) {
...
Router.prototype.applyRoutes = function (server, prefix) {
  ...
  methods.forEach(function (method) {
      ...
      self.routes[method].forEach(function (route) {
        ...
        options.url = concat(prefix, options.url);

        server.log.info('Registering %s at uri %s', method.toUpperCase(), options.url);
